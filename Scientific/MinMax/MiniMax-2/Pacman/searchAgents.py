  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not hitsWall:
        ...
        visitedCorners = (state[1][0] or nextCoordinates == self.corners[0], state[1][1] or nextCoordinates == self.corners[1], state[1][2] or nextCoordinates == self.corners[2], state[1][3] or nextCoordinates == self.corners[3])
        cost = 1
        successors.append(((nextCoordinates, visitedCorners), action, cost))
      
    self._expanded += 1
    
    hv = cornersHeuristic(state, self)
    for s in successors:
        (st, _, _) = s
        sh = cornersHeuristic(st, self)
        if hv > (1 + sh):
            print "state hv: ", hv
            print "state"
            (pm, grid) = state
